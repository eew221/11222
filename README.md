...
This repository accompanies manuscript v37, *Geometry-Defined Single-Owner PPE Evidence Allocation and Cross-Worker Contamination Analysis*.
...
- `manuscript/`: v37 manuscript source, bibliography, and non-sensitive analysis figures. The compiled submission PDF and qualitative source frames remain in the local submission workspace until publication authorization is confirmed.
...
- `audit/independent_worker_state_random_20260827_v1/results_v37_public/`: aggregate worker-state distributions, method summaries, filename-group summaries, selected-policy audit, and evaluation manifest.
...
6. For the worker-state audit, run `scripts/evaluate_worker_state_human_audit.py` only after the A/B passes and adjudication are frozen. The public `results_v37_public/` directory contains aggregate outputs from 686 consensus worker rows replayed over 15 seed--fold cells; the consensus CSV, annotated images, and per-row method outputs remain restricted.
...
The repository URL is https://github.com/eew221/11222. Release `v0.4.2` is archived at commit `6b916c59570ec5db1d068b00299ff8802b037846`, with Zenodo DOI `10.5281/zenodo.22119278`. A subsequent release must be created after the v37 materials are committed; do not cite the older DOI as if it archived v37 unless the release contents are verified to match.
