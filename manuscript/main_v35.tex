...
\item three complementary sampled human-ownership audits: a candidate-restricted difficult audit of 597 PPE boxes with third-expert adjudication, a pre-frozen source-stratified random open-candidate audit of 334 PPE boxes with three blind passes, and a detector-output audit of 376 rows with three blind passes; these audits are not mutually independent samples and all remain separate from the geometry-defined full-corpus worker-state reference.
\end{itemize}
...
\caption{RC-WSSI converts object detections into a geometry-defined single-owner representation. PPE evidence has one person owner, is mapped to safe/unsafe/review, and is evaluated under a declared filename-group-disjoint role split. The fixed high-to-low threshold diagnostic returns the most permissive retained threshold under its stated model; infeasible selection returns review-all. The full-corpus endpoint remains geometry-defined; three complementary sampled human-ownership audits are reported separately and do not establish a human-validated full-corpus worker-state endpoint.}
\label{fig:pipeline}
\end{figure*}
...
Three experts completed separate blind passes. For each predicted PPE row, an expert could select any displayed detected person, \texttt{OUTSIDE\_DETECTED\_PERSON\_SET}, \texttt{FALSE\_DETECTION}, or \texttt{AMBIGUOUS}; for each grey missed-reference row, the expert selected the visible owner, an owner outside the detected-person set, or \texttt{AMBIGUOUS}. Experts saw no allocation output, threshold, sealed reference, or other response. All three CSV files were frozen with SHA-256 digests before analysis. A majority label was used only when at least two experts agreed; a three-way disagreement remained \texttt{AMBIGUOUS}. This protocol measures sampled detector-output ownership and failure decomposition, not full-corpus semantic truth or deployment risk. The detector-output sample overlaps the difficult audit in four images and the random open-candidate audit in two images; the difficult and random samples are disjoint. The three audits are therefore complementary evidence with disclosed overlap, not independent replications.
...
\textbf{Code availability.} The experiment and analysis scripts, manuscript source, blank human-audit templates, environment specification, and derived non-image manifests are publicly available under the MIT License at \url{https://github.com/eew221/11222}; the earlier public package is tag \texttt{v0.4.1}. The submission artifact must be tag \texttt{v0.4.2}, containing the source that compiles this revision, aggregate random-audit diagnostics, the pre-frozen detector-output audit generator, the three-pass annotation service, and the post-freeze error-decomposition script. The tag's immutable archival DOI and commit hash must be inserted here before submission. The repository deliberately excludes original frames, image-derived overlays, weights, caches, and per-image labels. No DOI is claimed before that archival release exists.
