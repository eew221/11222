...
\textbf{Data availability.} The experiments reuse an existing low-light tunnel PPE image source. Original images, model weights, and prediction caches are not distributed in the public code repository. The authoritative dataset URL, license, access conditions, confirmation of permission to publish the qualitative frames, and the dataset's privacy/ethics or de-identification status must be verified and inserted by the authors before submission. The repository's \texttt{DATA\_AND\_ETHICS.md} records these required fields. Derived non-image manifests, blank annotation templates, and aggregate reporting materials may be released only to the extent allowed by the verified dataset license.

\textbf{Code availability.} The experiment and analysis scripts, manuscript source, blank human-audit templates, environment specification, and derived non-image manifests are available at \url{https://github.com/a450671132-code/122}. Before submission, the authors must create and cite a tagged immutable release of that repository and archive it with Zenodo (or an equivalent service); the resulting release identifier, commit hash, and DOI must be inserted here. This manuscript does not claim a DOI before that archival release exists.
