...
\item a blinded, independently adjudicated, candidate-restricted audit of 597 PPE-to-worker ownership decisions that separates semantic owner evaluation from the geometry-defined worker-state reference and compares four local allocation rules with all-to-all pooling.
\end{itemize}

The contribution is a transparent single-owner decision representation with a blinded, independently adjudicated association subset. It is not a new detector architecture, a claim that one local matcher is universally optimal, a full-corpus semantic annotation, a cluster-valid risk guarantee, or an end-to-end deployment safety guarantee.
...
The authors acknowledge the maintainers of the underlying tunnel PPE dataset and the Ultralytics open-source software ecosystem used in the experiments.
...
\textbf{Data availability.} The experiments reuse an existing low-light tunnel PPE image source. The authoritative dataset URL, license, access conditions, confirmation of permission to publish the qualitative frames, and the dataset's privacy/ethics or de-identification status are submission-blocking metadata that must be verified and inserted by the authors before submission; they are not recorded in the present workspace. Derived filename-group split manifests, group membership, detector metrics, prediction caches, frozen two-expert annotations, third-expert adjudications, final human reference labels, and result tables are archived with the reproducibility package, subject to the dataset license.

\textbf{Code availability.} The experiment and analysis scripts are included in the reproducibility package, including protocol audits, detector training, cache validation, human-reference association evaluation, paired group-resampling analysis, figure generation, and blinded adjudication. An archival repository URL, immutable release identifier, environment file, and artifact DOI are submission-blocking identifiers that must be inserted before submission; these externally accessible identifiers are not available in the present workspace.
