...
\abstract{Personal protective equipment (PPE) monitoring is commonly evaluated as object detection, although an operational representation must prevent one worker's evidence from being copied to another worker and should allow uncertain cases to be reviewed. We use the implementation label RC-WSSI for deterministic single-owner helmet and vest evidence allocation followed by safe, unsafe, or review states. Four filename-group-disjoint roles separate detector training, detector validation, threshold selection, and final testing across five seeds, three folds, and two non-nested detector-training label budgets. The fixed high-to-low Clopper--Pearson sequence is reported only as a model-conditional feasibility diagnostic under an idealized independent worker-level Bernoulli model; infeasible selection returns review-all. Against a geometry-defined operational convention, the 10\% YOLOv8s pipeline obtains unsafe F1 $0.727$, cell-mean automatic-safe rate $0.112$, and cell-mean composite miss-or-safe rate $0.172$. Person-weighted and filename-group-macro composite rates are $0.184$ and $0.243$, with an observed worst filename-group/fold value of $0.486$. A revision-added post hoc sensitivity that treats reference-review workers as unsafe during selection and scoring gives automatic-safe/composite rates of $0.151/0.305$. At $\alpha=0.05$, only 2/15 and 5/15 cells retain a threshold at 5\% and 10\% labels. Four complementary sampled audits are reported: a difficult candidate-restricted ownership audit, a random open-candidate ownership audit, a detector-output audit, and a pre-frozen random open-set human worker-state audit of 220 images and 686 worker rows. In the latter, RC-WSSI agrees with direct human overall-state labels on 0.570 of matched rows on average over 15 repeated seed--fold cells; the corresponding repeated-record pooled value is 0.568, and this conditional result is not a full-corpus or deployment estimate. The detector-output audit yields 305 predicted PPE rows and 71 reference PPE misses; its mutually exclusive predicted-row partition is 290 in-set owners, 7 outside-set owners, 7 false detections, and 1 ambiguous row, with 265/297 ($0.892$) determinate owner agreements. These audits support a single-owner representation and cross-worker-contamination analysis, not full-corpus semantic worker-state accuracy, universal association superiority, a cluster-valid statistical guarantee, or deployment safety.}
...
\item four complementary sampled human audits: a candidate-restricted difficult ownership audit of 597 PPE boxes with third-expert adjudication, a pre-frozen source-stratified random open-candidate ownership audit of 334 PPE boxes, a detector-output audit of 376 rows, and a pre-frozen random open-set worker-state audit of 686 worker rows from 220 images; these audits are not mutually independent samples and all remain separate from the geometry-defined full-corpus worker-state reference.
\end{itemize}
...
Table~\ref{tab:detector_robustness} reports the complete filename-group-disjoint YOLO11s repeat using the same 30 training, calibration, and final-test cells. At 10\% labels, YOLO11s produces worker-state F1 $0.719$, all-reference automatic-safe rate $0.104$, composite rate $0.169$, and coverage $0.803$, compared with $0.727/0.112/0.172/0.825$ for YOLOv8s. At 5\%, the corresponding YOLO11s values are $0.681/0.129/0.200/0.802$, compared with $0.677/0.136/0.205/0.791$. No YOLO11s cell returned review-all at the primary $(\alpha,\delta)=(0.30,0.10)$ setting. This same-corpus detector-variant comparison is a descriptive implementation check, not external-dataset validation or generalization evidence.

\begin{table*}[t]
\caption{Worker-state results for a same-corpus second-detector repeat under the filename-group-disjoint protocol. Values are means over 15 seed--fold cells at the primary $\alpha=0.30$, $\delta=0.10$ setting. Auto-safe is the all-reference automatic-safe rate used for calibration; composite includes unmatched unsafe reference persons.}
...
\subsection{Independent sampled human worker-state audit}\label{sec:humanstateaudit}
The worker-state audit was pre-frozen before inspecting detector outputs or the geometry-defined reference. It sampled 220 final-test images, 20 per each of 11 filename groups, and presented only the image and person boxes to two independent blind annotators. Each visible person was labeled directly as safe, unsafe, or review from the image; annotators could not select a PPE owner from a geometry-generated candidate list. A third expert adjudicated the three component-level disagreements after the two primary passes were frozen. The final consensus contains 686 worker rows: 164 safe, 342 unsafe, and 180 review. The two primary passes agree on the overall state for all 686 rows and on all components for 683/686 rows (0.996); the three component disagreements concern the vest label and were adjudicated as safe. Overall state is a direct human field and is not mechanically recomputed from the component fields.

We replayed every frozen 10\% detector cache on the audit images. For each of five seeds and three folds, the test cache belonging to the image's declared fold was used, and the selected threshold was recomputed from that cell's calibration split. Table~\ref{tab:humanstateaudit} reports the resulting worker-state comparison. ``Matched exact'' is conditional on a human-labeled person having a matched detector person; an unmatched detector person is retained as unmatched in the row-level file and in the composite diagnostic. The cell mean averages the 15 seed--fold cells. The repeated-record pooled columns sum the same 686 human rows over five seeds, so their denominators are repeated records rather than independent workers. These values are descriptive and do not convert the geometry-defined full-corpus endpoint into an independent population estimate.

\begin{table*}[t]
\caption{Independent sampled human worker-state audit on 220 pre-frozen images and 686 human-labeled worker rows. The detector cache and selected policy are evaluated separately for each of 15 seed--fold cells. Matched exact and decision rate are means over cells. The repeated-record pooled rates use the same sampled rows repeated over five seeds; they are not independent-worker estimates. Auto-safe and composite errors use the 1,710 repeated human-unsafe rows as denominator.}
...
\begin{tabular}{@{}l c c c c c c@{}}
\toprule
Method & Matched exact & Pooled exact & Decision rate & Auto-safe & Composite & Human-unsafe $n$ \\
\midrule
RC-WSSI lexicographic & 0.570 & 0.568 & 0.603 & 576/1710 (0.337) & 755/1710 (0.442) & 1710 \\
Center-inside & 0.571 & 0.568 & 0.604 & 571/1710 (0.334) & 750/1710 (0.439) & 1710 \\
Hungarian-IoU & 0.570 & 0.568 & 0.605 & 580/1710 (0.339) & 759/1710 (0.444) & 1710 \\
Max-IoU & 0.563 & 0.561 & 0.596 & 572/1710 (0.335) & 751/1710 (0.439) & 1710 \\
Global pooling & 0.292 & 0.301 & 0.351 & 428/1710 (0.250) & 607/1710 (0.355) & 1710 \\
\bottomrule
\end{tabular}}
\end{table*}

The independent state audit therefore gives a different result from the ownership audits. Local rules have similar conditional state agreement (cell means 0.563--0.571), while global pooling is lower (0.292) and often changes the review behavior. The local-rule state errors are substantial because this audit includes detector misses, person matching failures, localization errors, and confidence thresholds; it should not be summarized as a validated deployment state accuracy. The audit is a sampled complement to, rather than a replacement for, the geometry-defined full-corpus analysis.
...
The main practical result is not a large F1 gain. Under the geometry-defined operational convention, the proposed rule remains close to alternative local geometric allocation rules, while global pooling produces much higher common-threshold contamination diagnostics and frequent review-all outcomes after calibration. The difficult audit with third-expert adjudication sharpens this message: RC-WSSI, center-inside, and Hungarian-IoU all exceed 0.96 exact owner-set accuracy, and the paired descriptive intervals do not separate RC-WSSI from the latter two. The random open-candidate audit reports 320/321 conditional exact owner sets but the conservative all-row value is 320/334, explicitly accounting for 13 unresolved labels. The detector-output audit then exposes a different boundary: RC-WSSI matches 265/297 determinate predicted-PPE owners, with false detections, owner-outside-set events, and missed PPE retained as explicit failure categories. Global pooling instead creates 2,864 extra owner links and no exact owner sets on the difficult audit. Single-owner conditioning is therefore supported as a contamination-control mechanism and as a sampled ownership representation, without implying that the lexicographic matcher is universally best or that the full-corpus worker-state reference is independent semantic truth.
...
\textbf{Code availability.} The experiment and analysis scripts, v37 manuscript source, blank human-audit templates, environment specification, and derived non-image manifests are publicly available under the MIT License at \url{https://github.com/eew221/11222}. The prior immutable release \texttt{v0.4.2} resolves to commit \texttt{6b916c59570ec5db1d068b00299ff8802b037846} and is archived as a versioned Zenodo record at \url{https://doi.org/10.5281/zenodo.22119278}; the v37 source and aggregate outputs are identified by the immutable commit of the release accompanying this submission. The repository and archive deliberately exclude original frames, image-derived overlays, weights, caches, and per-image labels.
